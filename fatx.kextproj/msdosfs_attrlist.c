    ...
    struct ucred *cred)			/* Credentials (used for ATTR_CMN_USERACCESS) */
{
    ...
    if (a)
    {
        ...
        if (a & ATTR_CMN_CRTIME) {
            dos2unixtime(dep->de_CDate, dep->de_CTime, 0, attrptr);
        ...
        if (a & ATTR_CMN_ACCTIME) {
            dos2unixtime(dep->de_ADate, dep->de_ATime, 0, attrptr);
            ++((struct timespec *)attrptr);
        }
    ...
    if (a)
    {
        ...
        if (a & ATTR_VOL_SIGNATURE) *((u_long *)attrptr)++ = 0x5842;
...
int msdosfs_getattrlist(struct vop_getattrlist_args *ap)
{
    ...
    /*
     * Make sure caller isn't asking for an attribute we don't support.
     */
    ...
    void *attrbufptr)
{
    ...
    {
        /* FIXME: Determine how FATX stores the volume label */
#if 0
        u_int16_t c;
